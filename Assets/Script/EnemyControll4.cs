...
public class EnemyControll4 : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (currentHp <= 0 && Enemy == false) //HPが0
		{
			...
			GameObject.Find("UIManager").GetComponent<TextWritten>().hpslider4.SetActive(false);
			GameObject.Find("UIManager").GetComponent<TextWritten>().EnemeyNameText4.SetActive(false);
	...
	void OnTriggerExit(Collider other) //射程圏外
	{
	...
	void OnTriggerStay(Collider other) //射程圏内
	{
		if (other.gameObject.tag == "Player" && currentHp >= 1)
		{
			GameObject.Find("UIManager").GetComponent<TextWritten>().hpslider4.SetActive(true);
			GameObject.Find("UIManager").GetComponent<TextWritten>().EnemeyNameText4.SetActive(true);
