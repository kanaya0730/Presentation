...
public class EnemyControll8 : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (currentHp <= 0 && Enemy == false) //HPが0
		{
			...
			GetComponent<Animation>().CrossFade("dead", 0.2f);
			GameObject.Find("UIManager").GetComponent<TextWritten>().hpslider8.SetActive(false);
			GameObject.Find("UIManager").GetComponent<TextWritten>().EnemeyNameText8.SetActive(false);
	...
	void OnTriggerStay(Collider other) //射程圏内
	{
		if (other.gameObject.tag == "Player" && currentHp >= 1)
		{
			GameObject.Find("UIManager").GetComponent<TextWritten>().hpslider8.SetActive(true);
			GameObject.Find("UIManager").GetComponent<TextWritten>().EnemeyNameText8.SetActive(true);
			if (CheckAniClip("attack_short_001") == false) return;
